	...
	return (
		<div className={history === '/' ? 'header' : 'scroll'} id="header">
			...
			<div className="search-toggle" style={styles}>
				<input
					type="text"
					placeholder="Search accommodation by cities..."
				/>
			</div>
			{!isLoggedIn ? (
				<div className="header-items">
					...
					{shortlist.length === 0 ? (
						<a href="/shortlists" className="navbar-item">
							<span className="heart-number-zero">
								{shortlist.length}
							</span>
							<BsHeartFill fill="white" /> Shortlist
						</a>
					) : (
						<a href="/shortlists" className="navbar-item">
							<span className="heart-number">{shortlist.length}</span>
							<BsHeartFill fill="red" /> Shortlist
						</a>
					)}
