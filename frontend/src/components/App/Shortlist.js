...
const Shortlist = ({ shortlist, setShortlist, changeShortlist }) => {
	...
	useEffect(() => {
		...
		for (
			let i = 0;
			i < JSON.parse(localStorage.getItem('shortlist')).length;
			i++
		) {
			axios
				.get(
					`http://localhost:5001/api/getproperties/${
						JSON.parse(localStorage.getItem('shortlist'))[i]
					}`,
				)
				.then((res) => {
					array.push(res.data);
	...
	return (
		<div className="shortlist">
			...
			<div className="shortlist-counter">
				<h3>
					{shortlistLocal ? shortlistLocal.length : 0} property shortlisted
				</h3>
			</div>

			<div className="shortlist-body" style={{ backgroundColor: '#e5e5e5' }}>
				{homesShortlist &&
					homesShortlist.map((item) => (
						<City
							home={item}
							shortlist={shortlist}
